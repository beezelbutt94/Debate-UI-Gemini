...
> Everything below "Viral Trending (current app)" predates the pivot from
> ViralSync (paid-ad amplification) to Viral Trending (viral-gap analysis +
...
## Viral Trending (current app)

Findings from actually building and running the Viral Trending rebuild — all
...
|---|---|---|
| Next.js 16 removed the `middleware.ts` convention in favor of `proxy.ts` (one exported proxy function per project). This repo already had a `proxy.ts` (Viral Trending's disabled-by-default tenant routing); adding Clerk's `middleware.ts` alongside it hard-fails the build. | `next build`: `Error: Both middleware file "./middleware.ts" and proxy file "./proxy.ts" are detected.` | Merged Clerk's `clerkMiddleware` auth gating and the existing tenant-routing logic into a single exported `proxy` in `proxy.ts`. |
| `next lint` no longer exists as a command in Next.js 16 — the `lint` script was silently broken (parsed "lint" as a directory argument). | `npm run lint` -> `Invalid project directory provided, no such directory: .../lint`. | Installed `eslint` + `eslint-config-next`, added `eslint.config.mjs` (flat config), changed the script to `eslint .`. Now finds 2 real, pre-existing issues in the untouched Viral Trending scaffold (`app/dashboard/settings/domain/page.tsx`, `postcss.config.mjs`) — left alone, out of scope for this pass. |
...
2 pre-existing Viral Trending findings -- and, unlike every prior feature,
...
| `npm run lint` | clean except the 2 pre-existing Viral Trending findings |
